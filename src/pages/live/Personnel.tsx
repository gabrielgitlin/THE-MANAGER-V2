...
import type { CrewMember, CrewRole, Show } from '../../types';
...
export default function Personnel() {
  ...
  const handleAddShow = (memberId: number) => {
    ...
    const show = MOCK_SHOWS.find(s => s.id === Number(newShowId));
    if (!show) return;
  ...
  const handleExportContactPDF = () => {
    ...
    doc.text('Phone:', 25, 90);
    doc.text(selectedMember.phone || 'N/A', 70, 90);
    
    // Add email with icon placeholder
    doc.text('Email:', 25, 100);
    doc.text(selectedMember.email || 'N/A', 70, 100);
